...
class NFTMarketplaceASC1(NFTMarketplaceInterface):
    class Variables:
        ...
        asa_id = Bytes("ASA_ID")
        asa_valid_buyer_tkn = Bytes("ASA_VALID_BUYER_TKN")
    ...
    def app_initialization(self):
        ...
        return Seq([
            ...
            App.globalPut(self.Variables.asa_valid_buyer_tkn, Btoi(Txn.application_args[2])),
            Return(Int(1))
        ])
    ...
    def make_sell_offer(self):
        """
        Single application call with 1 arguments.
        - method_name
        :return:
        """
        ...
        # the owner is the one which is selling
        valid_seller = Txn.sender() == App.globalGet(self.Variables.asa_owner)
        # the call to the app has the correct number of arguments
        valid_number_of_arguments = Txn.application_args.length() == Int(1)

        # combine all previous conditions
    ...
    def buy(self):
        ...
        valid_buyer_tkn = Btoi(Gtxn[0].application_args[1]) == App.globalGet(self.Variables.asa_valid_buyer_tkn)
    ...
    def global_schema(self):
        return algosdk.transaction.StateSchema(num_uints=3, num_byte_slices=3)

    @property
    def local_schema(self):
        return algosdk.transaction.StateSchema(num_uints=0, num_byte_slices=0)
